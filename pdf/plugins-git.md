...
### <span class="green">gitsigns.nvim</span>

Показывает изменения (hunks) прямо в редакторе:
...
- **gitsigns:**
  - Используй визуальный режим для stage/reset сразу нескольких строк.
  - Быстро смотри diff (<kbd>&lt;leader&gt;hp</kbd>) и blame (<kbd>&lt;leader&gt;hb</kbd>) не выходя из редактора.
- **lazygit:**
  - Открывай lazygit в отдельном сплите или табе для удобства.
  - Можно быстро делать stage/unstage, коммиты, пуши, даже не зная всех git-команд наизусть.
